...
def tenant_view(require_module: str | None = None, require_perm: str | None = None):
    ...
    def decorator(view_func: Callable[[HttpRequest], HttpResponse]):
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if require_perm and not user_has_permission(user, require_perm, using=tenant_db):
                return render(
                    request,
                    "tenant_portal/forbidden.html",
                    {"tenant": tenant, "tenant_user": user, "reason": "You do not have permission to access this page."},
                    status=403,
                )
